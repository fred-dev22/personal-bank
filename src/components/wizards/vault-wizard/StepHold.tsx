...
export const StepHold: React.FC<{
  ...
  return (
    <div style={{ display: 'flex', gap: 32, width: 800, margin: '0 auto' }}>
      ...
      {/* Colonne droite : diagramme (placeholder) */}
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{
          ...
          <div style={{ fontWeight: 700, fontSize: 24 }}>${Number(vaultData.amount ?? vaultData.balance ?? 0) - (Number(vaultData.reserve) || 0) - (Number(vaultData.hold) || 0)}</div>
          ...
          <div style={{ marginTop: 16 }}>
            <div>Available: ${Number(vaultData.amount ?? vaultData.balance ?? 0) - (Number(vaultData.reserve) || 0) - (Number(vaultData.hold) || 0)}</div>
            <div>Reserve: ${vaultData.reserve || 0}</div>
            <div>Hold: ${vaultData.hold || 0}</div>
          </div>
        </div>
      </div>
    </div>
